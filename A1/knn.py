...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
  ...
  # use a naive set of nested loops over the traning and test data to compute 
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
  ...
  #  uses only a single Python loop over the training data.
  for i in range(num_train):
    diff = x_train_flat[i] - x_test_flat  # Broadcasting
    dists[i] = torch.sum(diff ** 2, dim=1)  # Vectorized computation
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
  ...
  # Formulate the Euclidean distance using two broadcast sums and a matrix multiply
  # $$ \text{dist} (a,b)^2 &= \sum_{i} (a_i - b_i)^2 \\
  # &= \sum_{i} a_i^2 - 2\sum_{i} a_i b_i + \sum_{i} b_i^2$$
  x_train_suqre_sum = torch.sum(x_train_flat ** 2, dim = 1, keepdim=True) # size(num_train, 1)
  x_test_suqre_sum = torch.sum(x_test_flat ** 2, dim = 1, keepdim=True).t() # size(1, num_test)
  cross_mul = torch.mm(x_train_flat, x_test_flat.t()) # size(num_train, num_test)
  
  dists = x_train_suqre_sum - 2*cross_mul + x_test_suqre_sum
  return dists


def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k=1):
  """
  ...
  # for each col j in num_test: we get the kth smallest, using torch.topk
  _, indices = torch.topk(dists, dim = 0, largest = False, k = k, sorted = True)
  # print indices for debug
  
  # get the labels
  labels = y_train[indices]
  
  # Statistical voting, using torch.mode, `torch.mode` will return the smallest value among the tied ones.
...
class KnnClassifier:
  ...
  def predict(self, x_test, k=1):
    ...
    # Compute the distances between train and test datasets
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    for validation_index in range(num_folds):
      x_train = torch.cat([fold for i, fold in enumerate(x_train_folds) if i != validation_index], dim=0)
      x_test = x_train_folds[validation_index]
      y_train = torch.cat([fold for i, fold in enumerate(y_train_folds) if i != validation_index], dim=0)
      y_test = y_train_folds[validation_index]
...
def knn_get_best_k(k_to_accuracies: dict):
  ...
  for k, accuracies_lst in k_to_accuracies.items():
    # compute the mean accuracy
    acc = statistics.mean(accuracies_lst)
    if (acc > max_accuracie):
      max_accuracie = acc
      best_k = k
